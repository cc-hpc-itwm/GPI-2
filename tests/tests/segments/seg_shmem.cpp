...
namespace
{
  struct shm_opened
    {
      shm_opened (std::string const &path):_fd (::
                                                shm_open (path.c_str (),
                                                          O_RDWR | O_CREAT,
                                                          S_IRWXU))
      {
  ...
  struct mmapped
    {
      mmapped (int fd, size_t size,
               int prot):_pointer (::mmap (0, size, prot, MAP_SHARED, fd, 0))
      {
    ...
    T * shared_memory (std::string const &path, off_t size)
  {
    ...
    return mmapped (truncated (shm_opened (path), size),
                    static_cast < size_t > (size), PROT_READ | PROT_WRITE);
  }
}
