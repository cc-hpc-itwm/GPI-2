...
main (int argc, char *argv[])
{
  ...
  ASSERT (gaspi_segment_create
          (10, 1024 * 1024, GASPI_GROUP_ALL, GASPI_BLOCK,
  ...
  ASSERT (gaspi_segment_create
          (2, 1024, GASPI_GROUP_ALL, GASPI_BLOCK, GASPI_MEM_INITIALIZED));
  counter++;
  ASSERT (gaspi_segment_num (&segment_num));
  assert (segment_num == counter);
  ...
  ASSERT (gaspi_segment_create
          (1, 1024, GASPI_GROUP_ALL, GASPI_BLOCK, GASPI_MEM_INITIALIZED));
  counter++;
  ASSERT (gaspi_segment_num (&segment_num));
  assert (segment_num == counter);
