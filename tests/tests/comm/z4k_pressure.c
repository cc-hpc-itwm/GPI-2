...
main (int argc, char *argv[])
{
  ...
  while (k <= RUNS)
  {
    ...
    ASSERT (gaspi_notify
            (0, (myrank + 1) % highestnode, 0, 1, 0, GASPI_BLOCK));
    ...
    ASSERT (gaspi_notify
            (0, (myrank + highestnode - 1) % highestnode, 1, 1, 0,
             GASPI_BLOCK));
    ...
    ASSERT (gaspi_read (0, offset_check, (myrank + 1) % highestnode,
                        0, offset_read, GB / 2, 0, GASPI_BLOCK));

    ASSERT (gaspi_read (0, offset_check + (GB / 2), (myrank + 1) % highestnode,
                        0, offset_read + (GB / 2), GB / 2, 0, GASPI_BLOCK));

    ASSERT (gaspi_wait (0, GASPI_BLOCK));

#ifdef DEBUG
    gaspi_printf ("Values %f %f %f \n", mptr_f[0],
                  mptr_f[offset_read / sizeof (float)],
                  mptr_f[offset_check / sizeof (float)]);
#endif
    ...
    while (j < GB / sizeof (float))
    {
      if (mptr_f[j] != mptr_f[offset_check / sizeof (float) + j])
      {
        gaspi_printf ("value incorrect %f-%f at %d \n",
                      mptr_f[j], mptr_f[offset_check / sizeof (float) + j], j);
        ret = -1;
        goto out;
      }
      j++;
