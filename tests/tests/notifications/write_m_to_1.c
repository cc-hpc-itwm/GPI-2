...
main (int argc, char *argv[])
{
  ...
  gaspi_size_t mem_size = 2 * SLOT_SIZE * (numranks - 1);

  if (myrank == 0)
  {
    printf
      ("Mem size: %lu (%.2f MB)\nProcs: %u Max Slot size %lu Iterations %d\n",
       mem_size, mem_size * 1.0f / 1024 / 1024, numranks,
       (gaspi_size_t) SLOT_SIZE, MAX_ITERATIONS);
#ifdef WITH_SYNC
  ...
  ASSERT (gaspi_segment_create
          (0, mem_size, GASPI_GROUP_ALL, GASPI_BLOCK, GASPI_MEM_INITIALIZED));
  ...
  for (cur_slot_size = SLOT_SIZE; cur_slot_size >= sizeof (float);
       cur_slot_size /= 2)
  {
    if (myrank == 0)
      printf ("===== Slot Size %lu ====\n", cur_slot_size);
    ...
    for (iter = 0; iter < MAX_ITERATIONS; iter++)
    {
      ...
      if (myrank == 0)
      {
        for (i = 1; i < numranks; i++)
        {
          ...
          ASSERT (gaspi_write (0, offset_in, i,
                               0, 0, cur_slot_size, 0, GASPI_BLOCK));
      ...
      if (myrank != 0)
      {
        ...
        ASSERT (gaspi_write_notify (0, offset_in, 0,
                                    ...
                                    myrank, 1, 0, GASPI_BLOCK));
#else
        ASSERT (gaspi_write (0, offset_in, i,
                             0, offset_out, cur_slot_size, 0, GASPI_BLOCK));
#endif
        ASSERT (gaspi_wait (0, GASPI_BLOCK));
      }
#ifdef WITH_SYNC
      ...
      if (myrank == 0)
      {
        ...
        const gaspi_size_t total_elems =
          (cur_slot_size * (numranks - 1) / sizeof (float));
