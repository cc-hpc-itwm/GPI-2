...
main (int argc, char *argv[])
{
  ...
  ASSERT (gaspi_segment_create
          (seg_id, nprocs * 2 * transfer_size, GASPI_GROUP_ALL, GASPI_BLOCK,
  ...
  for (i = 0; i < nprocs; i++)
  {
    ...
    for (not = 0; not < avail_notifications; not++)
    {
      ...
      gaspi_notification_id_t the_notification =
        (gaspi_notification_id_t) (rank * avail_notifications + not);

      ASSERT (gaspi_write_notify (seg_id, offset, i,
                                  seg_id, offset, transfer_size,
                                  the_notification, 1, 0, GASPI_BLOCK));
    }
  }
