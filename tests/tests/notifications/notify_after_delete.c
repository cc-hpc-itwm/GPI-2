...
/* Test that does a notify to a remote segment without the local
   (deleted) */
...
main (int argc, char *argv[])
{
  ...
  ASSERT (gaspi_segment_create
          (seg_id, 1024, GASPI_GROUP_ALL, GASPI_BLOCK,
  ...
  if (rank == 0)
  {
    ...
    ASSERT (gaspi_segment_delete (seg_id));
    ...
    for (n = 0; n < notif_num; n++)
    {
      for (i = 1; i < nprocs; i++)
      {
        ...
        ASSERT (gaspi_notify (seg_id, i, notif, i, 0, GASPI_BLOCK));
      }
    }
  }
  else
