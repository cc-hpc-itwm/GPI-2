...
main(int argc, char *argv[])
{
  ...
  ASSERT (gaspi_segment_create(0, passive_max_msg_size, GASPI_GROUP_ALL, GASPI_BLOCK, GASPI_MEM_INITIALIZED));
  ...
  if( myrank == 0 )
    ...
    {
      gaspi_rank_t sender;
      ASSERT(gaspi_passive_receive(0, 0, &sender, passive_min_msg_size, GASPI_BLOCK));
      int elem;
  ...
  if( myrank == 0 )
    ...
    {
      gaspi_rank_t sender;
      ASSERT(gaspi_passive_receive(0, 0, &sender, passive_max_msg_size, GASPI_BLOCK));
