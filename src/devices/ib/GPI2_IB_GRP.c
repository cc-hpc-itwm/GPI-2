...

{
  ...
  const int dsize = args->f_args.elem_size * elem_cnt;
  ...
  if(rank < 2 * rest)
      ...
      else
	{
	  ...
	  if(args->f_type == GASPI_OP)
	    {
	      gaspi_operation_t op = args->f_args.op;
	      gaspi_datatype_t type = args->f_args.type;

	      fctArrayGASPI[op * 6 + type] ((void *) send_ptr, local_val, dst_val,elem_cnt);
	    }
	  else if(args->f_type == GASPI_USER)
	    {
	      args->f_args.user_fct (local_val, dst_val, (void *) send_ptr, args->f_args.rstate, elem_cnt, args->f_args.elem_size, timeout_ms);
	    }
