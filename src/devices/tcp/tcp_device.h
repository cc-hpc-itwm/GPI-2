...
typedef struct
{
  ...
  struct
  {
    ...
    enum
    {
      RECV_HEADER, RECV_TOPOLOGY, RECV_RDMA_WRITE, RECV_RDMA_READ,
      RECV_SEND
    } opcode;
...
struct tcp_cq *tcp_dev_create_cq (int, struct tcp_passive_channel *);

void tcp_dev_destroy_cq (struct tcp_cq *);

struct tcp_queue *tcp_dev_create_queue (struct tcp_cq *, struct tcp_cq *);

void tcp_dev_destroy_queue (struct tcp_queue *);

struct tcp_passive_channel *tcp_dev_create_passive_channel (void);

void tcp_dev_destroy_passive_channel (struct tcp_passive_channel *);

int tcp_dev_return_wc (struct tcp_cq *, tcp_dev_wc_t *);

int tcp_dev_init_device (struct tcp_dev_args *args);

int tcp_dev_stop_device (int);

int tcp_dev_is_valid_state (unsigned short);
