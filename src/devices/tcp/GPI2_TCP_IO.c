  ...
  tcp_dev_wr_t wr =
    {
      ...
      .target      = rank,
      .local_addr  = (uintptr_t) (glb_gaspi_ctx.rrmd[segment_id_local][glb_gaspi_ctx.rank].data.addr + NOTIFY_OFFSET + offset_local),
      .remote_addr = (glb_gaspi_ctx.rrmd[segment_id_remote][rank].data.addr + NOTIFY_OFFSET + offset_remote),
      .length      = size,
      .swap        = 0,
      .compare_add = 0,
  ...
  tcp_dev_wr_t wr =
    {
      ...
      .remote_addr = (glb_gaspi_ctx.rrmd[segment_id_remote][rank].data.addr + notification_id * sizeof(gaspi_notification_t)),
		       ...
		       gaspi_size_t * const size, const gaspi_queue_id_t queue)
{
  ...
  for (i = 0; i < num; i++)
      ...
      tcp_dev_wr_t wr =
	{
	  ...
	  .source      = glb_gaspi_ctx.rank,
	  .target        = rank,
	  .local_addr  = (uintptr_t) (glb_gaspi_ctx.rrmd[segment_id_local[i]][glb_gaspi_ctx.rank].data.addr + NOTIFY_OFFSET + offset_local[i]),
	  .remote_addr = (glb_gaspi_ctx.rrmd[segment_id_remote[i]][rank].data.addr + NOTIFY_OFFSET + offset_remote[i]),
	  .length      = size[i],
	  .swap        = 0,
	  .compare_add = 0,
	  .opcode      = POST_RDMA_WRITE
